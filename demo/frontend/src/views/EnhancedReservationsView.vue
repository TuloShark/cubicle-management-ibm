  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      if (route.params.date && typeof route.params.date === 'string') {
        console.log('EnhancedReservationsView - initializing from route date:', route.params.date);
        initializeFromRoute(route.params.date);
      } else {
        console.log('EnhancedReservationsView - using current global date:', selectedDate.value);
      }
    ...
    watch(selectedDate, async (newDate) => {
      console.log('EnhancedReservationsView - selectedDate changed to:', newDate);
    ...
    watch(selectedDateString, async (newDateString) => {
      console.log('EnhancedReservationsView - selectedDateString changed to:', newDateString);
      await fetchCubiclesForDate(selectedDate.value);
    });
...
.grid-container {
  ...
  border: none;
  width: 100%;
  /* Responsive overflow strategy - hide scrollbars on large screens unless needed */
  overflow: hidden;
  max-height: 80vh;
  margin-top: 0.5rem;
  box-sizing: border-box;
...
/* Enable scrolling on tablets and smaller screens */
...
/* Custom scrollbar styling - only visible on tablets and mobile */
@media (max-width: 768px) {
...
/* Firefox scrollbar styling for tablets and mobile */
@media (max-width: 768px) {
  .grid-container {
